...
Vec2 Device::Project(const Vec3& coord, const Mat4& transMat)
{
    ...
    float x = point.x * _bmp->PixelWidth + _bmp->PixelWidth / 2.0f;
    float y = -point.y * _bmp->PixelHeight + _bmp->PixelHeight / 2.0f;
    return Vec2(x, y);
}
...
void Device::Render(Camera* camera, const std::vector<Mesh*>& meshes)
{
    ...
    Mat4 projectionMat4;
    Mat4::createPerspective(0.78f, (float)_bmp->PixelWidth / _bmp->PixelHeight, 0.01f, 1.0f, &projectionMat4);
...
//                                                      mesh->Rotation.X, mesh->Rotation.Z) *
        ...
        Mat4 transformMat4 = worldMat4 * viewMat4 * projectionMat4;

        for (auto&& vertex : mesh->Vertices)
        {
            // First, we project the 3D coordinates into the 2D space
            Vec2 point = Project(vertex, transformMat4);
